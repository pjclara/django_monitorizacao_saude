...
from healthData.consumers import NotificationConsumer

class NotificationConsumerTestCase(TestCase):
    def test_notification_consumer(self):
        # Create a test WebSocket communicator
        communicator = WebsocketCommunicator(
            URLRouter([re_path(r'^ws/notifications/$', NotificationConsumer.as_asgi())]),
            '/ws/notifications/'
        )

        # Connect to the WebSocket
        connected, _ = self.get_response(communicator.connect())

        # Assert that the connection was successful
        self.assertTrue(connected)

        # Send a test message to the WebSocket
        message = {'type': 'notification', 'data': 'Test notification'}
        self.get_response(communicator.send_json_to(message))

        # Receive the message from the WebSocket
        response = self.get_response(communicator.receive_json_from())

        # Assert that the received message matches the sent message
        self.assertEqual(response, message)

        # Close the WebSocket connection
        self.get_response(communicator.disconnect())

    def get_response(self, coroutine):
        """
        Helper method to get the response from an asynchronous coroutine.
        """
